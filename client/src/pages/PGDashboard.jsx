...
const PGDashboard = ({ brandTitleOverride }) => {
  ...
  const fetchPgCaseSheetHistory = async () => {
    try {
      ...
      // Filter rows to the current PG department (show only Periodontics when appropriate)
      const targetDeptKey = String(pgDepartmentKey || user?.department || '').trim().toLowerCase();
      ...
      console.debug('[PGDashboard:DEBUG] visibleRows count:', visibleRows.length);
            ...
            {activeView === 'my-appointments' && (
              <div className="pg-my-appointments">
                ...
                {Array.isArray(myUpcomingAppointments) && myUpcomingAppointments.length > 0 && (
                  <div className="pg-assigned-cases-table-wrapper">
                    <table className="pg-assigned-cases-table" style={{ fontSize: '0.85em', tableLayout: 'fixed', width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr style={{ padding: '4px' }}>
                          <th style={{ padding: '8px 6px', textAlign: 'center', fontWeight: 600 }}>S.No</th>
                          <th style={{ padding: '8px 6px', textAlign: 'center', fontWeight: 600 }}>Doctor</th>
                          <th style={{ padding: '8px 6px', textAlign: 'center', fontWeight: 600 }}>Patient Name</th>
                          <th style={{ padding: '8px 6px', textAlign: 'center', fontWeight: 600 }}>Patient ID</th>
                          <th style={{ padding: '8px 6px', textAlign: 'center', fontWeight: 600 }}>Date & Time</th>
                          <th style={{ padding: '8px 6px', textAlign: 'center', fontWeight: 600 }}>Complaint</th>
                          <th style={{ padding: '8px 6px', textAlign: 'center', fontWeight: 600 }}>Action</th>
                        </tr>
                      </thead>
                      <tbody>
                        {myUpcomingAppointments.map((appointment, index) => {
                          ...
                          return (
                            <tr key={bookingId || `${appointment?.patientId}-${appointment?.appointmentDate}`} className="pg-assigned-row">
                              <td style={{ padding: '8px 6px', textAlign: 'center', whiteSpace: 'nowrap' }}>{index + 1}</td>
                              <td
                                style={{ padding: '8px 6px', textAlign: 'left', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                              ...
                              <td
                                style={{ padding: '8px 6px', textAlign: 'left', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                                title={formatAppointmentComplaintDisplay(appointment?.chiefComplaint) || '—'}
                              >
